...
### 3.1: Mosquitto e Paho
O mosquitto configurado pelo projeto em assets/mosquitto.conf ignora conceitos fundamentais de segurança, como a autenticação dos clientes via usuário e senha pois contém a configuração allow_anonymous valorada como true, também ignora restrição de postagem por usuário ou assinatura de tópicos por usuário. Configuramos a conexão como websocket e reservamos a porta 9001, não explicítamos o ip pois por default ele é 127.0.0.1.
O versão do Paho para Javascript utilizada é a 1.1.0. Ao publicarmos as mensagens com a biblioteca, não estamos definindo o QoS pois por default o QoS do envio de mensagens é 0. Também não estamos informando o QoS ao assinarmos tópicos pois por default o QoS definido é 0. Já na conexão com o Mosquitto estamos definindo o id do cliente como o id do cliente e não estamos definindo o path pois o path definido por default pelo Pahot é /mqtt.
